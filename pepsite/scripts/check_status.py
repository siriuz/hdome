    ...
    for en in sorted( bi1.mdict.keys(), key = lambda(a) : int(a) ):
      try:
        ...
        with open( filepath, 'rb' ) as f:
            allpeps = f.readlines()
        count = len(allpeps) -1 # remember header line
        dbcount = IdEstimate.objects.filter( ion__experiment__title = bi1.mdict[en]['Experiment name'].strip() ).count()
        dbcount2 = Ion.objects.filter( experiment__title = bi1.mdict[en]['Experiment name'].strip() ).count()
        print bi1.mdict[en]['Experiment name'].strip(), filepath, count, dbcount, dbcount2 
      except:
